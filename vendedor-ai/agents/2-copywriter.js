...
3. Não mencione "vi suas imagens" ou "analisei seus posts" — fale como se você conhecesse o trabalho dele naturalmente.`;
  ...
  const estruturaIdeal = isAPI
    ? `COMO ESCREVER A MENSAGEM:\nEscreva como uma pessoa real que encontrou esse perfil e manda uma DM genuina — nao um vendedor, nao um bot.\nNAO use [HOOK], [VALOR], [PERGUNTA] — sao labels internos, NUNCA aparecem no texto.\n\nFluxo natural (3 a 5 frases, sem mais):\n- Abre com algo especifico do lead: uma dor tecnica que vc identificou, algo da bio, do nicho\n- Apresenta o que vc fez de forma simples e casual ("fiz uma API que...", "resolvi isso aqui pra um cara de...")\n- Menciona um resultado concreto de forma solta, como em conversa\n- Fecha com UMA pergunta simples e direta\n\nEXEMPLO BOM — copie o tom:\n"Seus flows quebram quando o telefone chega fora do formato?\nFiz uma API que converte qualquer variacao BR pra E.164 em ~50ms — ja integra direto no Make, n8n e Zapier.\nVale testar?"\n\nEXEMPLO RUIM — NUNCA faca assim:\n"Ola! Vi seu perfil incrivel! Tenho uma solucao perfeita que pode transformar seu negocio. Posso apresentar?"\n\nLINGUAGEM: portugues BR casual — "vc", "tbm", "aqui", "kk" se caber — nao forcado, natural.\nMINIMO: 3 linhas. MAXIMO: 5 linhas. Mensagem de 1 linha = INVALIDO.\nPROIBIDO: free, gratis, plano, preco, R$, $, sem cartao, trial, desconto.`
    : `COMO ESCREVER A MENSAGEM:\nEscreva como uma pessoa real que encontrou esse perfil e manda uma DM genuina — nao um vendedor, nao um bot.\nNAO use [HOOK], [VALOR], [PERGUNTA] — sao labels internos, NUNCA aparecem no texto.\n\nFluxo natural (3 a 5 frases, sem mais):\n- Abre com algo especifico do lead: um detalhe do conteudo, do nicho, do link da bio\n- Conecta ao que vc faz de forma simples ("fiz uma LP pra...", "ajudo criadores de [nicho] a...")\n- Menciona um resultado concreto de forma solta, como em conversa\n- Fecha com UMA pergunta simples e direta\n\nEXEMPLO BOM — copie o tom:\n"Seu conteudo de nutricao funcional ta consistente de verdade.\nNotei que o link da bio ainda e o Linktree padrao — fiz uma LP dark mode pra uma nutri aqui em SP, agendamento subiu 40% em 3 semanas.\nPosso te mandar o link do projeto?"\n\nEXEMPLO RUIM — NUNCA faca assim:\n"Oi! Vi seu incrivel perfil! Tenho uma oportunidade especial pra voce alavancar sua presenca digital. Vamos conversar?"\n\nLINGUAGEM: portugues BR casual — "vc", "tbm", "aqui", "kk" se caber — nao forcado, natural.\nMINIMO: 3 linhas. MAXIMO: 5 linhas. Mensagem de 1 linha = INVALIDO.\nPROIBIDO: preco, R$, $, plano, gratis, free, desconto, investimento.`;
  ...
  // ---- CONTEXTO DE MENSAGEM ANTERIOR (modo regeneração) ----
  const previousMsg = process.env.PREVIOUS_MESSAGE || null;
  const previousMsgContext = previousMsg
    ? `\nMENSAGEM ANTERIOR (não repita — melhore o ângulo, o gancho e a personalização):\n"${previousMsg.slice(0, 400)}"\nREGRA: a nova mensagem_1 DEVE ser claramente diferente e de maior qualidade que a anterior acima.\n`
    : '';

  const prompt = `Voce e uma pessoa real que encontrou um perfil interessante no Instagram e quer mandar uma DM autentica — sem parecer vendedor, sem parecer bot.\n\n${contexto_produto}\n\nDADOS DO LEAD:\n- Nicho: ${a.nicho}\n- Tipo: ${a.tipo_negocio}\n- Problema: ${a.problema_principal}\n- Consciencia: ${a.nivel_consciencia}\n- Angulo: ${a.angulo_abordagem}\n- Objecoes: ${JSON.stringify(a.objecoes_previstas)}\n- Motivo produto: ${a.motivo_produto}\n- Prioridade: ${a.prioridade}\n${postsContextWithRule}${previousMsgContext}\n\n${estruturaIdeal}\n${memoriaStr}\nREGRAS FINAIS:\n1. NUNCA coloque @handle no texto da mensagem\n2. NAO comece com \"Vi seu perfil\", \"Parabens\", \"Ola!\", \"Oi!\" sozinhos como abertura\n3. NUNCA use: \"solucao\", \"investimento\", \"beneficios\", \"diferenciais\", \"oportunidade\", \"incrivel\", \"transformar\"\n4. Followups: 2-3 linhas naturais, sem @handle, sem pitch de vendas\n5. Use \\\\n para quebras de linha no JSON\n6. Varie o angulo entre as 3 mensagens — nao repita o mesmo gancho\n\nRetorne SOMENTE o JSON (sem markdown, sem backticks, sem texto fora do JSON):\n{\n  \"mensagem_1\": {\"texto\": \"<texto natural aqui, 3-5 linhas>\", \"angulo\": \"...\", \"temperatura\": \"direta\"},\n  \"mensagem_2\": {\"texto\": \"<texto natural aqui, 3-5 linhas>\", \"angulo\": \"...\", \"temperatura\": \"suave\"},\n  \"mensagem_3\": {\"texto\": \"<texto natural aqui, 3-5 linhas>\", \"angulo\": \"...\", \"temperatura\": \"curiosidade\"},\n  \"mensagem_recomendada\": \"1\",\n  \"motivo_recomendacao\": \"...\",\n  \"followup_dia_3\": \"2-3 linhas naturais\",\n  \"followup_dia_7\": \"2-3 linhas com valor concreto\",\n  \"followup_dia_14\": \"2-3 linhas com oferta especifica\"\n}`;
